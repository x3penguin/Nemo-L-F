...
def store_potential_matches(found_item_id, lost_item_id, confidence, distance=None):
    ...
    try:
        ...
        ).where(filter=FieldFilter("lostItemId", "==", lost_item_id))
        ...
        if len(existing_matches) > 0:
            ...
            if confidence > existing_confidence:
                ...
                print(
                    f"Updated potential match between {found_item_id} and {lost_item_id} with higher confidence"
                )
        else:
            ...
            print(
                f"Stored new potential match between {found_item_id} and {lost_item_id}"
            )
...
def get_found_items():
    ...
    query = db.collection("items").where(filter=FieldFilter("status", "==", "FOUND"))
    docs = query.stream()
    items = []
    for doc in docs:
        item_data = doc.to_dict()
