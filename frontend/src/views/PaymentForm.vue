      ...
      <div class="payment-card">
        ...
        <div class="payment-form">
          ...
          <div class="payment-actions">
            <!-- Back Button -->
            <button
              @click.prevent="$router.push('/logistics-form')"
              type="button"
...
export default {
  ...
  async created() {
    ...
    let orderData =
      this.$route.params.orderData ||
      JSON.parse(localStorage.getItem("orderData"));

    // Check for query parameters if no stored data is found
    ...
    // Ensure amount is properly parsed as a float
    const shippingCost = parseFloat(orderData.amount) || 0.0;
    ...
    console.log("Order Details in Payment Form:", this.orderDetails);
  ...
  methods: {
    ...
    getUserIdFromToken(token) {
      try {
        ...
        return (
          decoded.id || decoded.userId || decoded.sub || decoded.uid || null
        );
      } catch (error) {
    ...
    async processPayment() {
      ...
      try {
        ...
        const createPaymentResponse = await fetch(
          "https://personal-kbjnamsn.outsystemscloud.com/NEMOPaymentService/rest/paymentAPI/create-payment-intent",
          {
            method: "POST",
            headers: {
