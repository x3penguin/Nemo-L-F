    ...
    <div class="container">
      <div class="collection-container">
        ...
        <div v-else-if="error" class="error-alert">
          <p>{{ error }}</p>
          <button @click="fetchMatchedItems" class="btn btn-primary btn-sm">Try Again</button>
        </div>
        ...
        <div v-else-if="!matchedItems.length" class="empty-state">
          ...
          <p>When one of your lost items is found and matched, it will appear here for collection.</p>
          ...
          <div class="matched-items">
            <div v-for="item in filteredItems" :key="item.id" class="item-card">
              <div class="item-image">
                <img :src="item.image_url || '/img/placeholder-image.jpg'" :alt="item.name">
              ...
              <div class="item-details">
                ...
                <p class="item-date">Matched: {{ formatDate(item.matched_date) }}</p>
              </div>
              <div class="item-actions">
                ...
                <div v-else-if="item.status === 'RETRIEVED'" class="retrieved-message">
        ...
        <div class="modal-container" @click.stop>
          ...
          <div class="modal-body">
            ...
            <div v-if="modalType === 'initiate'" class="collection-form">
              <p class="modal-description">Please select how you would like to collect your item:</p>
                  ...
                  <div class="saved-addresses">
                    ...
                    >
                      <div class="address-details">
                        <h4 class="address-name">{{ formatAddressName(address) }}</h4>
                        <p class="address-line">{{ formatAddressLine(address) }}</p>
                        <span v-if="address.is_default" class="default-badge">Default</span>
                      </div>
                      <div class="address-selection">
                        <div class="radio-button" :class="{ selected: selectedAddress && selectedAddress.id === address.id }"></div>
                      </div>
                    </div>
                  </div>
                  
                  <button @click="showNewAddressForm = true" class="btn btn-secondary btn-small" v-if="!showNewAddressForm">
                    + Add New Address
                  </button>
                </div>
                
                <div v-if="showNewAddressForm || savedAddresses.length === 0" class="new-address-form">
                  <div v-if="savedAddresses.length > 0" class="form-header">
                    <h4>New Address</h4>
                    <button @click="showNewAddressForm = false" class="btn btn-text">Cancel</button>
                  </div>
                  ...
                  <div class="form-group">
                    ...
                    >
                    <div v-if="addressErrors.street_address" class="error-message">{{ addressErrors.street_address }}</div>
                  </div>
                  ...
                  <div class="form-group">
                    ...
                    >
                    <div v-if="addressErrors.unit_number" class="error-message">{{ addressErrors.unit_number }}</div>
                  </div>
                  ...
                  <div class="form-row">
                    <div class="form-group half">
                      ...
                      >
                      <div v-if="addressErrors.postal_code" class="error-message">{{ addressErrors.postal_code }}</div>
                    </div>
                    ...
                    <div class="form-group half">
                      ...
                      >
                      <div v-if="addressErrors.city" class="error-message">{{ addressErrors.city }}</div>
                    </div>
                  </div>
                  ...
                  <div class="form-group">
                    ...
                    >
                    <div v-if="addressErrors.phone" class="error-message">{{ addressErrors.phone }}</div>
                  </div>
                  
                  <div class="form-check">
                    <input type="checkbox" id="default" v-model="newAddress.is_default">
                    <label for="default" class="check-label">Set as default address</label>
                  </div>
            ...
            <div v-else-if="modalType === 'details'" class="collection-details">
              <div class="item-summary">
                <div class="item-image">
                  <img :src="selectedItem.image_url || '/img/placeholder-image.jpg'" :alt="selectedItem.name">
                </div>
                <div class="item-info">
                  <h3>{{ selectedItem.name }}</h3>
                  <p><strong>Category:</strong> {{ selectedItem.category }}</p>
                  <p><strong>Status:</strong> <span class="status-badge" :class="'status-'+selectedItem.status.toLowerCase()">{{ formatStatus(selectedItem.status) }}</span></p>
                </div>
              </div>
              ...
              <div class="collection-info">
                ...
                <div class="detail-row">
                  ...
                  <span class="detail-value">{{ formatCollectionMethod(collectionDetails.type) }}</span>
                </div>
                
                <div v-if="collectionDetails.type === 'SELF_PICKUP'" class="pickup-details">
                  <div class="detail-row">
                    <span class="detail-label">Pickup Location:</span>
                    <span class="detail-value">{{ collectionDetails.pickup_venue }}</span>
                  </div>
                  
                  <div class="detail-row">
                    <span class="detail-label">Address:</span>
                    <span class="detail-value">{{ collectionDetails.pickup_address || 'Not specified' }}</span>
                  </div>
                  
                  <div class="detail-row">
                    <span class="detail-label">Available From:</span>
                    <span class="detail-value">{{ formatDate(collectionDetails.pickup_start) }}</span>
                  </div>
                  
                  <div class="detail-row">
                    <span class="detail-label">Available Until:</span>
                    <span class="detail-value">{{ formatDate(collectionDetails.pickup_end) }}</span>
                  </div>
                  
                  <div class="detail-row">
                    <span class="detail-label">Instructions:</span>
                    <span class="detail-value">{{ collectionDetails.pickup_instructions || 'No special instructions' }}</span>
                  </div>
                ...
                <div v-else-if="collectionDetails.type === 'COURIER'" class="courier-details">
                  <div class="detail-row">
                    <span class="detail-label">Status:</span>
                    <span class="detail-value status-badge" :class="'delivery-'+collectionDetails.status.toLowerCase()">{{ formatDeliveryStatus(collectionDetails.status) }}</span>
                  </div>
                  
                  <div class="detail-row">
                    <span class="detail-label">Delivery Address:</span>
                    <span class="detail-value">{{ formatFullAddress(collectionDetails) }}</span>
                  </div>
                  
                  <div class="detail-row">
                    <span class="detail-label">Estimated Delivery:</span>
                    <span class="detail-value">{{ formatDate(collectionDetails.estimated_delivery) }}</span>
                  </div>
                  
                  <div v-if="collectionDetails.courier_provider" class="detail-row">
                    <span class="detail-label">Courier:</span>
                    <span class="detail-value">{{ collectionDetails.courier_provider }}</span>
                  </div>
                  
                  <div v-if="collectionDetails.tracking_id" class="detail-row">
                    <span class="detail-label">Tracking ID:</span>
                    <span class="detail-value">{{ collectionDetails.tracking_id }}</span>
                  </div>
                  
                  <div class="delivery-progress">
                    <div class="progress-step" :class="{ active: isDeliveryStepActive('PAID') }">
                      <div class="step-dot"></div>
                      <div class="step-label">Payment Confirmed</div>
                      <div class="step-time" v-if="collectionDetails.payment_time">{{ formatTime(collectionDetails.payment_time) }}</div>
                    </div>
                    
                    <div class="progress-line" :class="{ active: isDeliveryStepActive('IN_PROGRESS') }"></div>
                    
                    <div class="progress-step" :class="{ active: isDeliveryStepActive('IN_PROGRESS') }">
                      <div class="step-dot"></div>
                      <div class="step-label">Pickup Scheduled</div>
                      <div class="step-time" v-if="collectionDetails.pickup_time">{{ formatTime(collectionDetails.pickup_time) }}</div>
                    </div>
                    
                    <div class="progress-line" :class="{ active: isDeliveryStepActive('COMPLETED') }"></div>
                    
                    <div class="progress-step" :class="{ active: isDeliveryStepActive('COMPLETED') }">
                      <div class="step-dot"></div>
                      <div class="step-label">Delivered</div>
                      <div class="step-time" v-if="collectionDetails.delivery_time">{{ formatTime(collectionDetails.delivery_time) }}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="payment-summary">
                ...
                <div class="fee-details">
                  ...
                  <div class="fee-row">
                    ...
                    <span>${{ (paymentDetails.distance_fee || 0).toFixed(2) }}</span>
                  </div>
                ...
                <p class="payment-description">
                  Click the button below to proceed to our secure payment gateway. Once payment is completed, your delivery will be scheduled.
                </p>
    ...
    setup() {
          ...
          matchedItems.value = [
            {
              ...
              venue: 'Central Mall',
              specific_location: 'Food court, 3rd floor',
              status: 'MATCHED',
              matched_date: '2023-03-15T14:30:00Z',
              image_url: null
            },
            {
              id: 2,
              ...
              venue: 'Bus #36',
              specific_location: 'Rear seat',
              status: 'COLLECTING',
              matched_date: '2023-03-10T09:15:00Z',
              image_url: null
            },
            {
              id: 3,
              ...
              venue: 'City Library',
              specific_location: 'Study area',
              status: 'RETRIEVED',
              matched_date: '2023-02-28T16:45:00Z',
              retrieved_date: '2023-03-02T11:20:00Z',
              image_url: null
            }
          ];
        } catch (err) {
      ...
      const fetchCollectionDetails = async (itemId) => {
        try {
          ...
          collectionDetails.value = {
            ...
            type: 'COURIER',
            status: 'IN_PROGRESS',
            pickup_venue: 'Central Mall',
            pickup_address: '123 Main Street, Level 1, Singapore 123456',
            pickup_start: '2023-03-16T10:00:00Z',
            pickup_end: '2023-03-16T18:00:00Z',
            pickup_instructions: 'Approach the information counter and mention the item ID',
            delivery_address: '456 Orchard Road, #12-34, Singapore 654321',
            estimated_delivery: '2023-03-17T14:00:00Z',
            courier_provider: 'FastDelivery',
            tracking_id: 'FD987654321',
            payment_time: '2023-03-16T09:30:00Z',
            pickup_time: '2023-03-16T11:45:00Z',
            delivery_time: null
          };
        } catch (err) {
      ...
      const saveAddress = () => {
        ...
        if (newAddressObj.is_default) {
          savedAddresses.value.forEach(a => { a.is_default = false; });
        }
